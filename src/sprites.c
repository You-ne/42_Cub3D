...
/*   Updated: 2021/02/20 06:56:38 by antoine          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_calc_col_sprite(t_coor a, t_coor pos, t_coor sp, t_img *tex)
{
	...
	{
		return (a.y > sp.y ? (int)((float)tex->width * (((siz / 2) - pos.dist)
		/ siz)) : (int)((float)tex->width * (((siz / 2) + pos.dist) / siz)));
	}
	else if ((int)pos.x < (int)sp.x)
	{
		return (a.y < sp.y ? (int)((float)tex->width * (((siz / 2) - pos.dist)
	...
	{
		return (a.x < sp.x ? (int)((float)tex->width * (((siz / 2) - pos.dist)
		/ siz)) : (int)((float)tex->width * (((siz / 2) + pos.dist) / siz)));
	}
	else if ((int)pos.y < (int)sp.y)
	{
...
t_coor *ft_add_sprite(t_game *game, t_coor ray, t_coor dir, t_coor eqline)
{
	...
	*sp = ft_sprite_dist(game->map, ray, dir, game->player.pos);
	...
	game->player.pos) : (float)ft_sprite_col(*sp, eqline, tex, game->player.pos);
